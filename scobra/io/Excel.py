...
def ReadExcel(excel_file, parse="cobra_string", Print=False, cores=None, single=False):
    ...
    % model         cobrapy model """
    if single:
        return ReadExcelSingle(excel_file, parse, Print)
    
    excel = pandas.ExcelFile(excel_file)
    #these separate parse call are I/O bounds so cannot be threaded
    ...
    args= []
    for _, rows in metabolites.iterrows():
        args.append((metabolite_dic, element_re, Print, rows))

    if cores==None:
        cores = multiprocessing.cpu_count()
    
    #multiprocessing metabolites
    with ThreadPoolExecutor(max_workers=cores) as metExecutor:
        futures = []
        for _, rows in metabolites.iterrows():
            futures.append(
                metExecutor.submit(metaboliteProcessing,
                                   (metabolite_dic,
                                    element_re,
                                    Print,
                                    rows)))
    ...
    with ThreadPoolExecutor(max_workers=cores) as reacExecutor:
        futures = []
        for _, rows in reactions.iterrows():
            futures.append(
                reacExecutor.submit(reactionProcessing,
                                   (model,
                                    reaction_list,
                                    metabolite_dic,
                                    Print,
    ...
    with ThreadPoolExecutor(max_workers=cores) as reacObjExecutor:
        reacObjExecutor.map(reactionObjectiveProcessing, [(i, reactions) for i in reaction_list])
...
def metaboliteProcessing(args):
    (metabolite_dic, element_re, Print, met_row) = args
...
def reactionProcessing(args):
    (model, reaction_list, metabolite_dic, Print, reac_row) = args
...
def reactionObjectiveProcessing(args):
    reaction, reactions = args
    reac_row = reactions.loc[reaction.id]
...
def ReadExcelSingle(excel_file, parse="cobra_string", Print=False):
    """ parse = "cobra_string" | "cobra_position"
    cobra_string
    % INPUT
    % fileName      xls spreadsheet, with one 'Reaction List' and one 'Metabolite List' tab
