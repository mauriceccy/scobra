...
class model(cobra.Model):
    ...
    def __init__(self, existing_model=None, bounds=1000.0):
        if type(existing_model) == model:
            ...
            self.solution = LegacySolution(None)
...
######################################################
    ...
    def GetSol(self, IncZeroes=False, AsMtx=False, sol=None, FixSumReacs=True,
        ...
        if not sol:
            ...
            sol = self.Solve(PrintStatus=False)
 
            NewFluxDic = sol.fluxes.to_dict()
            TempFluxDic = {}
            if IncZeroes:
                print "Not processed for very small numbers..." 
                return NewFluxDic
            else:
                for reac in self.Reactions():
                    if NewFluxDic[reac] != 0:
                        TempFluxDic[reac] = NewFluxDic[reac]
                        if abs(TempFluxDic[reac]) < tol:
                            del TempFluxDic[reac]
                return TempFluxDic
        """else:
                    if abs(sol[reac]) < tol:
                        sol[reac] = 0.0
