...
class fva(dict):
    ...
    def __init__(self,bounds=float("inf")):
        self.bounds = bounds
...
#            if self[reac][0] != float("-inf") and self[reac][1] != float("inf"):
    ...
    def FluxRangeDiff(self, fd, thres=1):
        ...
        diffdirec = self.FluxDiffDirec(self, fd) # reac in different direc
        comparereacs = list(set(both1n2).difference(diffdirec))
        overlaps = self.FluxRangeOverlap(self, fd)   # reac with overlap flux range
